...
if [ "$EUID" -ne 0 ]; then
  echo "Bitte führen Sie dieses Script als root aus (sudo)."
  exit 1
fi

# Prüfen, ob apt verfügbar ist / Check if apt is available
if ! command -v apt >/dev/null 2>&1; then
  echo "Kein 'apt'-Paketmanager gefunden. Dieses Script funktioniert nur auf Debian-basierten Systemen."
  exit 2
fi
...
echo "Führe vollständige Aktualisierung durch ..."
apt full-upgrade -y
...
echo "Bereinige zwischengespeicherte Paketdaten ..."
apt clean
...
if [ -f /var/run/reboot-required ]; then
  echo "Ein Neustart des Systems ist erforderlich. Das System wird jetzt neu gestartet."
  reboot
fi
